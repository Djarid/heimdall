...
- **The ontology (invariant 3.11):** a deterministic classifier over a BFO spine plus four domains (communications, scheduling, finance, publication), the UNCLASSIFIED fail-safe, forward-chaining derivation, agent-scoped cross-domain flow-to-sink. **The ontology and all rules are asserted hand-authored, no model output on the authoring path.** Take care with the evidence: the AST guard (`ontology/nornir/symbolic_guard.py`, D68) proves no model is *imported or called* on the authorisation path, but that does NOT establish hand-authorship, because a rule drafted by a model and pasted in by hand leaves no import to detect. The guard closes the runtime hole; the authoring-provenance hole is separate and open (section 8 lists model-drafted rules as a redesign-forcing finding). The hand-authorship claim rests on the author's assertion plus git history, not on an independent attestation; treat it as asserted, not verified. Evidence: `ontology/`, `git log`, `ontology/OUTCOME.md`.
