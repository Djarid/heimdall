...
**Version:** 2.3
**Date:** August 2026
**Status:** a briefing for a hostile reviewer; revised after three hostile reviews. v2.1 acts on the second review's structural findings, and, more importantly, on its central point: it stopped polishing prose and built the false-inert measurement, which found a real break. v2.2 acts on the third (repository-access) review: it inverted the 3.1 guard from a residual egress blacklist to an allowlist (D71) and reduced the false-inert rate again while pinning down that its residual is bounded by invariant 3.1 (D72). See the changelogs at the end
...
| Wanted | Status |
|--------|--------|
| False-inert rate under adversarial phrasing (**the** bound) | Measured, and LARGE: 1/17 on the self-authored corpus, 13/30 (about 43 percent) on a larger scenario-authored independent corpus (5.2, D67, D77). Both are same-author lower bounds; a true third-party corpus is still No |
