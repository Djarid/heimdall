...
| D29 | Phase-1 ontology built to pass two attach tests: a second medium feeds existing types, and a second subject-matter domain attaches without editing the first or the spine | SETTLED | If either extension forces a change to what exists, the layering is wrong; the medium attach test is the one most easily forgotten because email arrives first. Structurally satisfied by the seed: media beyond email are present in `yggdrasil/media.py` and feed the same domain types (medium attach), and the domain layer sits under the spine ready for a sibling domain (domain attach) | section 4.2; `ontology/yggdrasil/media.py`, `ontology/yggdrasil/domain/communications.py` |
...
3. **Every NOT YET TESTED invariant has a construction path, and 3.9/3.11 are now partly demonstrated.** Invariants 3.6 (action-critical half), 3.9 (coverage bound) and 3.11 trace to D20 to D30, D38 and D43 to D49. The substrate spike demonstrated the action-critical mechanism substrate-neutrally (D43 to D45); the seed and Nornir demonstrate classification, the fail-safe, reasoner soundness and agent-scoped cross-domain flow-to-sink on a labelled corpus (D46 to D49). Invariant 3.9's coverage bound is measured for the first time (88.2 percent on the seed corpus). The live-store binding and coverage beyond the seed remain untested. No untested invariant lacks a build route.
...
6. **Threat surface is not narrowed by the staging choice.** D22a records that starting with email (a staging decision) does not narrow the threat model to email; D29's medium attach test is structurally satisfied by the seed (media beyond email present in `yggdrasil/media.py`, feeding the same types), so the ontology cannot silently become email-shaped. Checked against `ONTOLOGY_CONSTRUCTION.md` sections 2.4 and 4.
...
10. **The seed was tested, not asserted.** D46 to D49: the ontology and Nornir run against a hand-labelled corpus and pass the four obligations (`ontology/OUTCOME.md`). The result is reported with its coverage figure (88.2 percent), never as complete coverage. A downgrade, a fail-safe breach, an unsound derivation or a missed action-critical value would fail the suite; none occurred on this corpus.

**Known residual risk this log makes explicit:** the strongest live guarantee (structural separation, D02 to D09) is fully proven. The substrate mechanism (D25, D43 to D45) and the seed classifier and reasoner (D46 to D49) are demonstrated on small corpora, substrate-neutrally. The guarantee's *extent* still depends on ontology coverage growing beyond the seed, and on binding the proven algorithm to the live Memgraph store. The design is sound in mechanism, demonstrated on a seed, and unproven at full coverage. That is the honest state.
