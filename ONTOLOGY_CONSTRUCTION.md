...
This queue is now instrumented (decision D60). `NornirResult.coverage_gaps()` groups every review-routed assertion by the reason it needs review (`unclassified`, `unrecognised_request`, high-risk `tie`) with actionable samples, and the test harness reports it, so a human sees where coverage is thin without discovering it by luck. It reports, it does not act: growth stays hand-authored and human-curated. The gap capture asserts one hard property, that every review-routed assertion is accounted for, so nothing silently escapes it. The fourth domain, publication (D59), was the first coverage extension driven this way, closing the open-web surface (D22a). The gap capture is the Phase 1 to 2 precursor to Odin's later automated proposal path (7.2), which observes the same signal but is provenance-gated.
