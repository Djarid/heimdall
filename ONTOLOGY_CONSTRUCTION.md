...
A domain is a subject-matter area, not a source medium. Email, a PDF, an OCR'd image and a transcribed voicemail that express the same fact must land as the same typed assertion; this is the medium-blindness the architecture requires (HEIMDALL.md Phase 4). The parser at Bifröst sets the taint class (`EXTERNAL_COMMS`, `EXTERNAL_DOCUMENT` and so on); the domain layer sets the type. A "meeting request" is a scheduling-domain concept regardless of whether it arrived by email or by voicemail.

The anti-pattern to avoid by name: an "email ontology", a "PDF ontology", an "image ontology". Per-medium ontologies make medium-blindness impossible and give the same real-world fact incompatible types depending on how it arrived. Organise the domain layer around what the content is about, never around how it was parsed.
...
Phase 1 proves the separation on a single ingestion source (email) with read-only, human-gated autonomy and an empty action-critical set. The Phase 1 ontology is correspondingly small, and deliberately built so a second domain attaches later without disturbing it.

### 4.1 What Phase 1 builds

- **The shared spine.** A minimal upper layer (enough SUMO/BFO types to anchor the domain layer), the trust lattice (TAINTED, VOUCHED, TRUSTED, CANONICAL), and the action and constraint vocabularies. The action vocabulary in Phase 1 is small and read-only or human-gated (classify, triage, summarise, draft-for-review). The constraint vocabulary carries the axioms Phase 1 needs.
- **One domain layer: communications.** The email-sourced subject-matter types. The PoC's flat four-field schema (`sender_extracted`, `subject_extracted`, `requested_action_summary`, `entities`) is the seed. It becomes typed nodes under the communications domain, with a type hierarchy rather than a flat list, and each type carries the machinery for an `action_critical` declaration even though the Phase 1 action-critical set is empty.
- **The `UNCLASSIFIED` path.** Content that does not map to a known type is classified `UNCLASSIFIED_DATA_ASSERTION`, `actionable: false`, routed to human review, never trusted or actionable by default (HEIMDALL.md Nornir). This is the coverage-fail-safe and it must exist from the first day.
...
### 4.2 The attach test for a second domain

The Phase 1 ontology is correct only if a second domain (scheduling, say) can attach by adding a domain-type layer under the shared upper ontology, without editing the communications types and without touching the shared spine. If attaching a second domain forces a change to the first, the layering is wrong and the seed must be refactored before Phase 4. Building the seed with this attach test in mind is cheaper than discovering the coupling later.
