...
- **Sink-wiring honesty (NARROWED by D89 and D93).** The gate proves a declared wiring safe or unsafe. Historically it did not verify that a sink's declaration of how it consumes a field is truthful. This is now narrowed on three of the four scoped axes (`plans/declaration_attestation_scoping.md`). D89: sink consequentiality is DERIVED from a declared effect primitive over an attested table, not a per-sink boolean (direction B), and an action-critical value cannot be declared inert to slip the gate (direction A). D93: a sink's declared effect primitive is VERIFIED against its OBSERVED behaviour for every instrumentable sink (direction D), so a money mover declaring `display_only` is caught by evidence and blocked. What remains: the OPAQUE sink that cannot be instrumented (degrades to B plus C), the runtime-taint form of D, and direction C (attest WHO may declare, the config-tamper adversary). Invariant 3.6's action-critical determination stays bounded by these residuals and by ontology coverage; it is no longer bounded by the boolean-flag, inert-mode or (for observable sinks) wrong-primitive lie.
