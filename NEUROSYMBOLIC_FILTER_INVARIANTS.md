...
**Acceptance.** An AST scan of the authorisation-path packages (`ontology/yggdrasil`, `ontology/nornir`, `poc/symbolic.py`) asserts no model-client import, no inference call and no subprocess to a model runner, while permitting graph-database drivers (the substrate, not a model). It runs as a harness obligation (`ontology/nornir/symbolic_guard.py`, obligation 3.1 in `ontology/tests/harness.py`) and is verified to bite against a planted model import. Honest status of the automation: this AST check exists and runs in the harness; there is NO continuous-integration wiring yet, so nothing runs it automatically on commit or push. A repository-access review found that an earlier version of this acceptance clause described "static analysis in CI" and "the PoC's AST check" when neither existed (the PoC verified 3.1 by human inspection only); the check now exists, CI does not, and this clause states that plainly (decision D68).
