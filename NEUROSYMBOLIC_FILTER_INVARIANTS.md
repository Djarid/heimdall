...
**DEMONSTRATED on a four-domain seed; coverage breadth remains untested.** The mechanism the PoC wholly lacked now exists and its four acceptance obligations run and pass on a labelled corpus (decisions D46 to D62): a deterministic classifier over a typed ontology, the UNCLASSIFIED fail-safe, forward-chaining derivation with a soundness control, and agent-scoped cross-domain flow-to-sink feeding the Gjoll gate. The marshalling seam to the real model pipeline is proven end to end (D62). What remains NOT YET TESTED is coverage BREADTH: the guarantee is only as strong as the ontology's coverage, and that is measured (94.7 percent on the seed corpus) rather than complete. Sits logically alongside 3.2 (trust by origin): once content is stamped untrusted, the deterministic layer decides what it is. Maps to principle 6 (provenance is first-class), principle 10 (determinism is a property of the boundary) and Nornir plus Mímisbrunnr.
