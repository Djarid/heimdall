...
7. **Join a real model call behind the Rust client interface D105 and D106 already named, alongside the cognition stub rather than in its place, and re-run the target loop, now with a real model producing the proposed advisory content.** ~~Replace the cognition stub with a real model call~~ (this document's original wording, amended by build-order step seven, D115 to D117, and recorded as an amendment rather than as compliance in `DECISIONS.md` D116): the stub, `DefaultCognitionStep`, is retained, byte for byte unchanged in logic, rather than deleted, because an honest `TrustLevel::Tainted`/`ConsumeMode::Action` declaration for the model's own output means every model-authored proposal blocks at Gjöll's check five, the designed outcome of the step; if the stub had genuinely been replaced everywhere, nothing anywhere in the build would ever reach an executed outcome, and step six's own demonstration that the governed pipeline can authorise and execute a genuine action would have no live positive control at all on a machine with no model. The retention carries a stated expiry trigger, so the amendment is not open ended: Gjöll's own promotion and re-validation gate landing is the trigger, at which point deleting the stub becomes a single, clean edit rather than a load-bearing change. See `plans/dd/process-engine.md` sections 4 and 12, and `DECISIONS.md` D115 to D117, for the full account.
