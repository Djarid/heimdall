...
| Nornir (classifier and reasoner) | DEMONSTRATED, one RED obligation | `ontology/nornir/` real: `rules.py` (370 lines, four rule kinds, risk-tier priority, shared `carries_imperative_or_consequence` guard D69), `engine.py`, fail-closed `unclassified.py`; harness `ontology/tests/harness.py` (706 lines) runs obligations 8.1 to 8.4. Obligation 8.2 is failing: false-inert 1/17 self-authored, 16/33 (about 48 percent) on the independent corpus, 5/36 (about 14 percent) blind (D67, D77, D88, bounded by 3.1). | D67-fix's disposition is settled (D124); R-1's condition is re-scoped separately (D125). Coverage 36/38 on a self-authored four-domain seed; breadth untested; a true third-party corpus still wanted. |
...
**The pluggable, fail-closed classifier (D-2).** The classifier is a swappable component behind a stable interface. Its binding contract is fail-closed: unclassifiable or unconfidently-classified content routes to human review and is never trusted; the inert type is earned only by a positive informational signal, so a passively-phrased consequential message does not silently go inert. The open false-inert break (D67, measured 16/33 (about 48 percent) on the independent corpus, with the blind corpus reading 5/36 (about 14 percent), after the D69 and D72 structural guards reduced the self-authored figure from 3/12, suite deliberately red) is a tracked Phase-2 arming gate (risk R-1): no autonomous consequential capability may arm against Nornir's classification until D67-fix is closed and re-measured on a genuinely independent corpus. One scoped exception (D74): the fail-closed Fenrir + Huginn detection layer was built before the gate closes, since it arms no consequential capability. It was proposed as direction (d), the attempt-introspection catch, but was built and demoted (D75): a robust model is not steered by the false-inert payloads, so it emits no action attempt and the tripwire has nothing to catch, making it an injection-success detector rather than a false-inert fix. D124 settles the disposition: the candidate fixes (a deterministic referential-completeness discipline; a fail-closed advisory model that only routes to review; or accepting the bound) are DECLINED at layer one, on cost-and-invariant grounds, not on a smallness claim; the interface stays built to accept a fix or a better mechanism later without redesigning Nornir. R-1 stays OPEN and its gate condition, now known to be unsatisfiable as written, is re-scoped separately by D125.
...
**Build delta.** Real classifier and reasoner on a four-domain seed. Coverage is 36/38 self-authored; breadth is untested and an independent-labelled corpus does not exist (risk R-2). D67-fix's disposition is settled (D124, declined at layer one); the break stays open and red.
...
Inter-phase dependencies: Phase 2 depends on Phase 1's boundary; Phase 3's Gjöll depends on Phase 2's world model and flow-to-sink label; Phase 3 must not arm a consequential capability until R-1 (D67-fix) closes; that condition is now known to be unsatisfiable as written and is re-scoped separately (D124, D125), R-1 still OPEN; Phases 4 to 6 all depend on a real Phase 1 to 3 core.
...
|---|---|---|---|---|
| R-1 | False-inert classification break (D67); a consequential message earns an inert signal and skips the gate | OPEN, measured 16/33 (about 48 percent) on the independent corpus and 5/36 (about 14 percent) blind, bounded by 3.1, suite deliberately red | Phase 2 | Tracked arming gate: no autonomous consequential capability arms until D67-fix closes and is re-measured on a genuinely independent corpus (D-2). D124 declines the classifier-side fix at layer one on cost-and-invariant grounds; D125 names this gate's condition as unsatisfiable as written, because D67-fix will not close at layer one, and re-scopes it separately, still OPEN. Fix must be a design change, not more keywords. Scoped exception (D74): the fail-closed Fenrir + Huginn detection layer was built before the gate closes; it arms nothing. Proposed as candidate fix direction (d) but built and demoted (D75): a robust model is not steered, so it is an injection-success detector, not a false-inert fix. |
...
| 5.4 Nornir | Nornir | D-2; D31, D52, D54, D55, D69, D67, D124, D125; R-1, R-2 |
