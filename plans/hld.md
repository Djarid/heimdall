...
| Nornir (classifier and reasoner) | DEMONSTRATED, one RED obligation | `ontology/nornir/` real: `rules.py` (370 lines, four rule kinds, risk-tier priority, shared `carries_imperative_or_consequence` guard D69), `engine.py`, fail-closed `unclassified.py`; harness `ontology/tests/harness.py` (706 lines) runs obligations 8.1 to 8.4. Obligation 8.2 is failing: false-inert 1/17 self-authored, 13/30 (about 43 percent) on a larger independent corpus (D67, D77, bounded by 3.1). | Close or scope D67-fix (D-2); the break is large, not an edge case. Coverage 36/38 on a self-authored four-domain seed; breadth untested; a true third-party corpus still wanted. |
...
|---|---|---|---|---|
| R-1 | False-inert classification break (D67); a consequential message earns an inert signal and skips the gate | OPEN, measured 1/17 on an independent corpus (reduced from 3/12 by D69 and D72, bounded by 3.1), suite deliberately red | Phase 2 | Tracked arming gate: no autonomous consequential capability arms until D67-fix closes and is re-measured on a genuinely independent corpus (D-2). Fix must be a design change, not more keywords. Scoped exception (D74): the fail-closed Fenrir + Huginn detection layer was built before the gate closes; it arms nothing. Proposed as candidate fix direction (d) but built and demoted (D75): a robust model is not steered, so it is an injection-success detector, not a false-inert fix. |
