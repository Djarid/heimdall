...
- The known residual is the D67 false-inert break: consequential content that positively earns an inert signal is neither gated nor reviewed. It is measured, named and left red, and it is the arming gate of section 3.1, not a silent gap. Its classification-side fix is declined (D124); the gate's own condition is re-scoped separately (D125), still open.
...
- D67-fix is open as a break, its disposition settled DECLINED at layer one (D124). The classifier interface (section 3.1) is built to accept a fix or a better mechanism behind its fail-closed contract; no consequential capability arms against Nornir until R-1's gate condition, re-scoped separately (D125), is satisfied (risk R-1).
...
- **8.2 Classification correctness.** Each case to its expected type, with the failure direction distinguished: a downgrade of an action-critical value to an inert label is a critical finding that fails the suite, while an over-classification costs only a review and is tolerated. This obligation is deliberately red: false-inert 1/17 on the self-authored corpus, 16/33 (about 48 percent) on the independent corpus (D77) and 5/36 (about 14 percent) on the blind-authored corpus (D88), and it is left red because a suite that names a real break is worth more than a green one that never tested it. Its disposition is settled DECLINED at layer one (D124); the red bar is not a fix awaited but a fix declined. A future session must not mistake this red bar for a regression, and must not silence it with more keywords.
