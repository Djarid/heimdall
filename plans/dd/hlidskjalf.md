...
- **A minimal, unsigned, unchained, in-process append-only seam now exists in `himinbjorg`, satisfying HK-4's block-on-failed-log property only (D112, build-order step four of `plans/synthesis-bootstrap.md`).** `crates/himinbjorg/src/audit.rs` provides `DecisionRecorder`, a narrow one-method write contract, and `MinimalDecisionRecorder`, one concrete implementation with no update and no delete operation on an already-written entry, which is HK-2's structural form of append-only holding for this seam too. `himinbjorg::broker::broker_authorised_action` writes a decision and its six `CheckRecord`s through this seam before the actuator it gates is ever invoked, structurally rather than by comment, and a failed write refuses without invoking the actuator: exactly HK-4's load-bearing property, that an unlogged decision is treated as no decision. This is the whole of what this seam satisfies. `RecordedDecision`, the recorded entry's own shape, carries only the agent, the action, the decision and the six checks; `entry_id`, `timestamp`, `world_model_state_hash` and `signature`, the fields section 3.1 above names, are absent from it, not merely empty, because there is no Rust Hliðskjálf, no Rust Mímisbrunnr and no signing key at this fidelity for any of the four to be derived from. The signed chained service, `verify_chain`, the Gjallarhorn wiring on a verification failure and the causal-unwind anchoring (sections 3.2 to 3.5, 4 and 5 above) all remain unbuilt and stay the Phase-2 build this document specifies; the `DecisionRecorder` trait is this minimal seam's own extension point for that eventual replacement, not a stand-in for it. A named, load-bearing limit travels with this seam rather than being smoothed over: nothing in its type system stops a caller from supplying an implementation whose write reports success without retaining anything, which would defeat the audit obligation while looking, from `broker_authorised_action`'s side, identical to an honest write (REQ-35 and EC-13 of `.opencode/plans/git-actuator-step-four.md`; `plans/dd/actuator-git.md` section 8). See `DECISIONS.md` D112 and `plans/dd/actuator-git.md` for the full build record.
