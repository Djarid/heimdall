...
| 6 | Witness single use or replay resistance | **Narrowed by build-order step five (D113), unchanged and still narrowed at build-order step six (D114).** The process engine's fixed sequence obtains at most one witness per run and passes it to `broker_authorised_action` exactly once, so replay is unreachable **through the engine**; each of step six's five separate processes is one more such run, obtaining and spending one witness apiece, so the narrowing holds exactly as it did at step five. `broker_authorised_action` itself is unchanged and stays replayable by any other caller holding a witness (EC-12's open half). Closing it in full still needs the witness taken by value, or a nonce in the audit record, neither built by step five or step six |
| 7 | Concurrency safety for the actuator and the recorder | **Partly addressed by build-order step five (D113), within one process only; still unaddressed across processes after build-order step six (D114).** The engine sequences its own run start to finish inside a single process, so a second witness cannot appear mid-sequence from that same caller. Step six's own out-of-band driver runs its five engine invocations strictly sequentially, which is a property of the driver, not of the engine or the actuator, and demonstrates nothing about safety under concurrency: two concurrent engine *processes* against the same working repository remain unchanged and out of scope. The actuator still holds no lock and the minimal recorder is still not concurrency hardened (EC-10) |
| 8 | A third operation (merge), which D108's dogfooding text names alongside commit and push | Added as an operation variant when a real workflow needs it, per the Open/Closed analysis in section 9.1 of the build spec. **Build-order step six (D114) used `action:git.merge` as its negative case N1, deliberately, without prejudging this item.** The block N1 demonstrates is a statement about the cohort's attested control surface **today** (`hierarchy_vor::cohort::PERMITTED_ACTIONS` and `himinbjorg::definition::GLOBAL_DEFAULT_ACTIONS` do not carry `action:git.merge`) and about nothing else; it is not a ruling that merge should never be permitted, and this item stays exactly what it was: the anticipated third `GitOperation` variant, added when a real workflow needs it |
...
| 11 | The process engine's fixed five-step sequence | **Delivered by build-order step five (D113).** `crates/process-engine/` gives `broker_authorised_action` its first non-test caller, `sequence.rs`'s own single execute-step call site, and Himinbjörg's other three interfaces theirs alongside it. This advances invariant 3.6 only in the narrow sense the live detectors measure (a non-test caller now exists); it is not observed end-to-end containment (D91, D92) and it does not run the target loop (D108's own definition of done stays build-order step six) |
| 12 | Verification of the resolved `git` binary's identity | EC-2's trust root |
| 13 | `ActuationRefusal::PartialEffect` becoming reachable | **Still unreachable after build-order step five, and still unreachable after build-order step six (D114).** The engine executes exactly one operation per validated proposal, and step six ran it five separate times as five separate processes, one operation each (a commit, a push and three refusals), so no run holds both a commit outcome and a push outcome to combine even now that a real commit and a real push have both executed. `execute` still performs exactly one operation per call and `broker_authorised_action` still authorises exactly one operation per witness; the variant stays named and typed, not delivered behaviour, deferred to whichever later step first chains two operations behind one witness |
