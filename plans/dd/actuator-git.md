...
| 6 | Witness single use or replay resistance | **Narrowed by build-order step five (D113), not closed.** The process engine's fixed sequence obtains at most one witness per run and passes it to `broker_authorised_action` exactly once, so replay is unreachable **through the engine**; `broker_authorised_action` itself is unchanged and stays replayable by any other caller holding a witness (EC-12's open half). Closing it in full still needs the witness taken by value, or a nonce in the audit record, neither built by step five |
| 7 | Concurrency safety for the actuator and the recorder | **Partly addressed by build-order step five (D113), within one process only.** The engine sequences its own run start to finish inside a single process, so a second witness cannot appear mid-sequence from that same caller; two concurrent engine *processes* against the same working repository are unchanged and out of scope (step five's own EC-16). The actuator still holds no lock and the minimal recorder is still not concurrency hardened (EC-10) |
...
| 13 | `ActuationRefusal::PartialEffect` becoming reachable | **Still unreachable after build-order step five.** The engine executes exactly one operation per validated proposal and stages nothing itself (PE-3 of that step's own rulings), so no run holds both a commit outcome and a subsequent push outcome to combine even now that a caller exists. `execute` still performs exactly one operation per call and `broker_authorised_action` still authorises exactly one operation per witness; the variant stays named and typed, not delivered behaviour, deferred to whichever later step first chains two operations behind one witness |
